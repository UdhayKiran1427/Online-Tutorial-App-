...
function continueCourse(courseId) {
    ...
    // Find the course from the loaded courses
    const courseElement = document.querySelector(`[onclick="continueCourse(${courseId})"]`);
    const courseCard = courseElement ? courseElement.closest('.course-card') : null;
    
    if (courseCard) {
        const linkElement = courseCard.querySelector('a[href*="course"]');
        if (linkElement) {
                ...
                <div class="course-content">
                    ...
                    ${course.link && enrollment ? `
                        <div style="margin-top: 1rem;">
                            <a href="${course.link}" target="_blank" class="btn" style="display: inline-flex; align-items: center; gap: 0.5rem; text-decoration: none;">
                                🔗 Course Resources
                            </a>
                        </div>
                    ` : ''}
